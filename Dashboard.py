...
# ===============================================
    with tab4:
        ...
        # 2️⃣ CIERRE DE FACTURACIÓN (filtrado dinámico)
        ...
        # 3️⃣ COMERCIAL (filtrado dinámico)
        ...
        # 4️⃣ REQUERIMIENTO ESPECIAL (filtrado dinámico)
        requerimientos_disponibles = df_filtrado['REQUERIMIENTO ESPECIAL'].dropna().unique().tolist()
        requerimiento_seleccionado = st.multiselect("Selecciona el/los Requerimiento(s) Especial(es):", requerimientos_disponibles)

        if requerimiento_seleccionado:
            df_filtrado = df_filtrado[df_filtrado['REQUERIMIENTO ESPECIAL'].isin(requerimiento_seleccionado)]

        # 5️⃣ OBSERVACIONES (filtrado dinámico)
        observaciones_disponibles = df_filtrado['OBSERVACIONES'].dropna().unique().tolist()
        observaciones_seleccionadas = st.multiselect("Selecciona la(s) Observación(es):", observaciones_disponibles)
