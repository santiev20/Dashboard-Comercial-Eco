...
if archivo_excel:
    ...
    with tab2:
        ...
        compania_buscada = st.text_input("🔎 Buscar Compañía:", key='busqueda_compania_enviados')

        # 👉 Filtro por Compañía
        companias_disponibles = df_enviados['Compañía'].dropna().unique().tolist()
        compania_seleccionada = st.multiselect(
            "🏢 Filtrar por Compañía:",
            opciones := ["Todos"] + companias_disponibles,
            default="Todos",
            key='filtro_compania_enviados'
        )

        # 🔹 Filtrar DataFrame según los filtros aplicados
        df_enviados_filtrado = df_enviados.copy()

        # Filtro por compañía buscada (texto libre)
        if compania_buscada:
            df_enviados_filtrado = df_enviados_filtrado[
                df_enviados_filtrado['Compañía'].str.contains(compania_buscada, case=False, na=False)
            ]

        # Filtro por compañía seleccionada (de la lista)
        if "Todos" not in compania_seleccionada and compania_seleccionada:
            df_enviados_filtrado = df_enviados_filtrado[
                df_enviados_filtrado['Compañía'].isin(compania_seleccionada)
            ]

        # ===========================================
        # 👇 Código original (con el df filtrado)
        # ===========================================

        st.dataframe(df_enviados_filtrado.head())

        # Procesar datos
        df_enviados_filtrado['Dia'] = pd.to_datetime(df_enviados_filtrado['Dia'], errors='coerce')
        df_enviados_filtrado['Subtotal'] = pd.to_numeric(df_enviados_filtrado['Subtotal'], errors='coerce')
        df_enviados_filtrado = df_enviados_filtrado.dropna(subset=['Dia', 'Subtotal'])
        ...
        with col5:
            ...
            seleccion_periodos = st.multiselect(
                f"(Opcional) Selecciona los {opcion_enviados.lower()} que quieres sumar:",
        ...
        with col6:
            ...
            fig3.update_layout(
                xaxis_title=opcion_enviados,
                yaxis_title='Total enviados a facturar ($)',
                yaxis_tickformat=',.0f'
            )
...
# ===============================================
    with tab4:
        ...
        if not df_filtrado.empty:
            ...
            if 'Subtotal' in df_filtrado.columns:
                # Aquí sacamos el total filtrado directamente
                total_facturado = df_filtrado['Subtotal'].sum()

                st.metric("💰 Total facturado (según filtros aplicados)", f"${total_facturado:,.0f}")

                # ================== Gráfico de Torta ==================
                # Si seleccionas todos, se muestra el reparto. Si no, se muestra el único comercial filtrado.
    ...
    with tab6:
        df_aprovechables = pd.read_excel(archivo_excel, sheet_name=4)
